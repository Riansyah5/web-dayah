...
<head>
    ...
    <style>
        ...
        /* === RADIO OPTIONS === */
        .option-box { 
            cursor: pointer; padding: 20px 24px; border: 2px solid var(--border-soft); 
            border-radius: 16px; background: var(--surface);
            transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1); 
            display: flex; align-items: flex-start; gap: 16px; margin: 0;
        }
        .option-box:hover { 
            border-color: var(--c-3); background-color: var(--hover-bg); 
            transform: translateY(-2px); box-shadow: 0 6px 15px var(--shadow-color);
        }
        .option-indicator {
            width: 24px; height: 24px; border-radius: 50%; border: 2px solid var(--border-hard);
            display: flex; align-items: center; justify-content: center; flex-shrink: 0; margin-top: 4px; transition: all 0.2s ease;
        }
        .option-indicator::after {
            content: ''; width: 10px; height: 10px; border-radius: 50%;
            background: var(--surface); transform: scale(0); transition: transform 0.2s cubic-bezier(0.34, 1.56, 0.64, 1);
        }
        .option-radio:checked + .option-box { 
            border-color: var(--c-1); background: var(--hover-bg); box-shadow: 0 4px 14px var(--shadow-color);
        }
        .option-radio:checked + .option-box .option-indicator {
            background: var(--c-1); border-color: var(--c-1);
        }
        .option-radio:checked + .option-box .option-indicator::after { transform: scale(1); }

        /* === NAVIGATION GRID === */
        .nav-grid { display: grid; grid-template-columns: repeat(5, 1fr); gap: 10px; }
        .nav-btn { 
            aspect-ratio: 1; display: flex; align-items: center; justify-content: center;
            font-weight: 700; font-size: 0.95rem; border-radius: 12px; text-decoration: none; transition: all 0.2s;
        }
        .nav-empty { background: var(--surface); border: 2px solid var(--border-soft); color: var(--text-muted); }
        .nav-empty:hover { border-color: var(--c-3); color: var(--c-1); }
        .nav-answered { background: var(--answered-bg); border: 2px solid var(--c-2); color: var(--c-1); }
        .nav-current { background: var(--c-1); color: white; border: 2px solid var(--c-1); box-shadow: 0 4px 12px var(--shadow-color); }

        /* === TEXTAREA & BUTTONS === */
        .premium-textarea {
            background: var(--bg-body); border: 2px solid transparent; border-radius: 16px; padding: 20px; transition: all 0.2s; resize: vertical;
        }
        .premium-textarea:focus { background: var(--surface); border-color: var(--c-2); box-shadow: 0 0 0 4px var(--shadow-color); outline: none; }

        .btn-primary-custom {
            background: linear-gradient(135deg, var(--c-1) 0%, var(--c-2) 100%); color: white; border: none; border-radius: 100px;
            font-weight: 700; padding: 14px 32px; transition: all 0.2s;
        }
        .btn-primary-custom:hover { opacity: 0.9; color: white; transform: translateY(-2px); box-shadow: 0 10px 20px -10px var(--shadow-color); }
        
        .btn-outline-custom {
            background: white; color: var(--c-1); border: 2px solid var(--border-soft); border-radius: 100px; font-weight: 700; padding: 12px 32px; transition: all 0.2s;
        }
        .btn-outline-custom:hover { border-color: var(--c-3); background: var(--hover-bg); color: var(--c-1); }

        .btn-finish {
            background: var(--c-1); color: white; border: none; border-radius: 100px; font-weight: 800; padding: 16px 32px; transition: all 0.2s; letter-spacing: 0.5px;
        }
        .btn-finish:hover { opacity: 0.9; color: #fff; box-shadow: 0 8px 20px var(--shadow-color); }

        /* Indikator Simpan Kelas Dinamis */
        .save-indicator-box { font-size: 0.85rem; font-weight: 600; transition: all 0.3s; border: 1px solid transparent; }
        .state-saving { background: var(--answered-bg); border-color: var(--c-3); color: var(--c-1); }
        .state-saved { background: var(--bg-body); border-color: var(--border-soft); color: var(--text-muted); }

        /* Custom Scrollbar */
        ::-webkit-scrollbar { width: 6px; }
        ::-webkit-scrollbar-track { background: transparent; }
        ::-webkit-scrollbar-thumb { background: var(--c-3); border-radius: 10px; }
        ::-webkit-scrollbar-thumb:hover { background: var(--c-2); }
                ...
                <div class="surface-container p-4 p-md-5">
                    ...
                    <div class="mb-5">
                        ...
                        @if($currentAnswer->question->audio_file)
                            <div class="mt-4 p-2 rounded-pill border" style="background: var(--bg-body); border-color: var(--border-soft)!important;">
                                <audio controls class="w-100" style="height: 45px;"><source src="{{ asset('storage/'.$currentAnswer->question->audio_file) }}" type="audio/mpeg"></audio>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-5 pt-4 border-top border-light">
                        ...
                        @if($page < $totalQuestions)
                            <a href="?no={{ $page + 1 }}" class="btn-primary-custom text-decoration-none">
