                    ...
                    <tbody>
                        @forelse($schedules as $schedule)
                            @php
                                ...
                                // Styling Row
                                $rowClass = '';
                                if ($isAbsent && !$hasSubstitute) $rowClass = 'table-danger'; // Izin tapi blm ada badal (Bahaya)
                                if ($hasSubstitute) $rowClass = 'table-warning'; // Sudah ada badal (Aman)
                            @endphp
                                ...
                                <td>
                                    @if($hasSubstitute)
                                        <div class="d-flex align-items-center justify-content-between">
                                            ...
                                            <form action="{{ route('academic.picket.remove', $substituteData->id) }}" method="POST" onsubmit="return confirm('Batalkan badal ini?')">
                                                @csrf @method('DELETE')
                                                <button class="btn btn-sm text-danger ms-2" title="Hapus"><i class="bi bi-x-circle-fill"></i></button>
                                            </form>
                                        </div>
                                    @else
                                        <form action="{{ route('academic.picket.assign') }}" method="POST" class="d-flex gap-2">
                                            ...
                                            <select name="substitute_teacher_id" class="form-select form-select-sm" style="max-width: 200px;" required>
