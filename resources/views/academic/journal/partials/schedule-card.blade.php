<div class="card h-100 border-0 shadow-sm rounded-4 {{ $isBadal ? 'border-warning border-2' : '' }}">
  <div class="card-body p-4">
    ...
    @if($isDone)
    {{-- Jika sudah selesai, tombol tetap AKTIF tapi teksnya berubah --}}
    <div class="d-grid gap-2">
      <button class="btn btn-success rounded-pill disabled" style="opacity: 1;">
        <i class="bi bi-check-circle-fill me-2"></i> Jurnal Terisi
      </button>

      {{-- Tombol Edit Absensi --}}
      {{-- Kita perlu ID Jurnal untuk edit. Logic pengambilan ID harus dipastikan ada di Controller atau View Composer --}}
      {{-- Cara cepat: Ambil via Helper Model atau Query sederhana --}}
      @php
      $journalId = \App\Models\TeachingJournal::where('lesson_schedule_id', $schedule->id)
